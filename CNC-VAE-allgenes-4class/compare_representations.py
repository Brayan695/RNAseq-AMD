...
CNC_VAE_AMD_V5_allgenes_4class reference notebook, using this project's tf.keras
CNC-VAE instead of the notebook's PyTorch model, so the two can be compared side
by side.

Data: the FULL gene_input.csv transcriptome (~18,056 genes, all 453 donors),
joined with clinical/genotype covariates from MetaSheet_Processed.csv. Target is
the four-class MGS1..MGS4 AMD grade (0..3). This is a high-dimensional,
low-sample regime - overfitting risk is high, so the PCA/raw baselines below are
load-bearing, not just a formality.
...
from misc.dataset import DEFAULT_CLIN_FILE, DEFAULT_GENE_INPUT_FILE, get_data
...
def main():
    ...
    d = get_data(args.gene_input_file, args.clin_file, clinical_mode=args.clinical_mode)
    y = d['y']
