...
from misc.dataset import AMDDataset, DEFAULT_CLIN_FILE, DEFAULT_GENE_INPUT_FILE
...
parser = argparse.ArgumentParser(
    description='Train a CNC-VAE integrating the FULL gene_input.csv transcriptome (~18,056 '
                 'genes, all 453 donors) with clinical/genotype covariates '
                 '(MetaSheet_Processed.csv), for the four-class MGS1..MGS4 AMD grade task. '
                 'Mirrors the CNC_VAE_AMD_V5_allgenes_4class notebook. High-dimensional, '
                 'low-sample regime - overfitting risk is high, treat the PCA/raw baselines '
                 'in compare_representations.py as load-bearing.'
)
...
parser.add_argument('--numfolds', help='Number of stratified CV folds (ignored when --fold 0)',
                     type=int, default=5)
parser.add_argument('--gene_input_file', help='Path to gene_input.csv (all 453 donors, all genes)',
...
if __name__ == '__main__':
    ...
    dataset = AMDDataset(
        args.gene_input_file, args.clin_file, args.label_col,
