...
class XDEC:
    ...
    original X-DEC (which fine-tunes the xvae encoder jointly with the
    ...
    than the original: does the pretrained xvae embedding, as-is, already
    cluster well, without X-DEC's joint encoder fine-tuning helping it along?
    Port of clustering_functions.dec_vae_cluster / cluster_mlp_vae."""
    ...
    def fit(self, s1, s2, y=None, maxiter=2000, batch_size=64, update_interval=50,
        ...
        # This is the one change from the original X-DEC: everything below
        # still runs the real X-DEC self-training algorithm (K-means init, Q/P
        # soft-assignment refinement) - it just never touches the encoder.
        for p in self.xvae.net.parameters():
