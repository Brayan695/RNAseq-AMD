...
class DEC:
    ...
    original CNC-DEC's DEC (which fine-tunes the CNCVAE encoder jointly with the
    clustering head), this version clusters on top of an already-trained,
    ...
    than the original: does the pretrained CNC-VAE embedding, as-is, already
    cluster well, without DEC's joint encoder fine-tuning helping it along?
    Port of clustering_functions.dec_vae_cluster, but for the single
    concatenated-input CNCVAE rather than the dual-branch xvae (see
    X-DEC/models/dec.py:XDEC for that variant)."""
    ...
    def fit(self, s1, s2, y=None, maxiter=2000, batch_size=64, update_interval=50,
        ...
        # This is the one change from the original CNC-DEC DEC: everything
        # below still runs the real DEC self-training algorithm (K-means init,
        # Q/P soft-assignment refinement) - it just never touches the encoder.
