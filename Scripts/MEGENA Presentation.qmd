...
# Set doPerm to FALSE to bypass stringent filtering
pfn = calculate.PFN(corr)

# Check if the PFN was successfully created
if (nrow(pfn$edges) == 0) {
  stop("The PFN could not be created. No edges were found. You may need to use a different file or adjust MEGENA parameters.")
} else {
  message(paste("PFN created successfully with", nrow(pfn$edges), "edges."))
}

# 4. Take the edges from the PFN
pfn_g = igraph::graph_from_data_frame(d = pfn$edges, directed = FALSE)
