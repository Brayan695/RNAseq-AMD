...
parser.add_argument('--normalization', type=str, default='minmax', choices=['minmax', 'log1p_minmax'],
                     help="RNA preprocessing matching the checkpoint's original training script: "
                          ...
                          "(e.g. the BCEWL_encoder_* checkpoints)")
...
def main():
    ...
    if x_clin.shape[1] + x_mrna.shape[1] != ae.args.input_size:
        raise ValueError(
            'Input width {} does not match checkpoint input_size {} - '
            'check --clinical_mode matches how this encoder was trained'.format(
                x_clin.shape[1] + x_mrna.shape[1], ae.args.input_size)
        )

    z = ae.predict(x_clin, x_mrna)  # frozen - predict() already wraps eval()+no_grad internally
    y = d['y']
    ...
    fig.suptitle('CNC-VAE classifier head (latent_dim={}) - {}-fold CV, out-of-fold predictions'.format(
        latent_dim, args.n_splits))
    fig.tight_layout()
    plot_path = os.path.join(args.out, 'cncvae_classifier_curves.png')
    fig.savefig(plot_path, bbox_inches='tight')
    print('Saved', plot_path)

    result_path = os.path.join(args.out, 'cncvae_classifier_result.json')
    with open(result_path, 'w') as f:
        json.dump({
            'model': 'CNC-VAE', 'encoder': args.encoder, 'latent_dim': int(latent_dim),
