...
def load_cncvae_model(path):
    ...
    returned object's .predict()/.train() etc. all work immediately - matches
    the load_cncvae_model()/load_xvae_model()/load_autoencoder() convention
    already used in ../CNC-DEC, ../X-DEC, and ../DEC.
    """
    ckpt = torch.load(path, map_location='cpu')
