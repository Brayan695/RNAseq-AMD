# Loads a previously-trained DEC (plain MLP) autoencoder (no retraining) and
# trains a small binary classifier head on top of its FROZEN latent embedding,
# to predict mgs_level (MGS1 vs MGS4) directly from the compressed
# representation.
#
# The classifier head's architecture (models/classifier_head.py) is picked
# automatically from the loaded encoder's own latent size (--neurons_e the
# checkpoint was trained with): 16->4->1, 8->4->1, or plain 4->1. Only the
# head's weights are ever trained - the encoder is used purely for inference
# (ae.predict()).
#
...
parser = argparse.ArgumentParser(
    description='Train a binary classifier head on top of a frozen, already-trained DEC '
                "autoencoder's latent embedding, evaluated via stratified k-fold CV "
...
def main():
    ...
    latent_dim = ae.args.neurons_e
    print('Loaded encoder: neurons_h={} neurons_e={} n_features={}'.format(
        ae.args.neurons_h, latent_dim, ae.args.n_features))
    ...
    if X.shape[1] != ae.args.n_features:
        raise ValueError(
            'Input width {} does not match checkpoint n_features {} - '
            'check --clinical_mode matches how this encoder was trained'.format(
                X.shape[1], ae.args.n_features)
        )

    z = ae.predict(X)  # frozen - predict() already wraps eval()+no_grad internally
    y = d['y']
    ...
    fig.suptitle('DEC classifier head (latent_dim={}) - {}-fold CV, out-of-fold predictions'.format(
        latent_dim, args.n_splits))
    fig.tight_layout()
    plot_path = os.path.join(args.out, 'dec_classifier_curves.png')
    fig.savefig(plot_path, bbox_inches='tight')
    print('Saved', plot_path)

    result_path = os.path.join(args.out, 'dec_classifier_result.json')
    with open(result_path, 'w') as f:
        json.dump({
            'model': 'DEC', 'encoder': args.encoder, 'latent_dim': int(latent_dim),
