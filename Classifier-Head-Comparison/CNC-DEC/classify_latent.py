# Loads a previously-trained CNC-VAE encoder (no retraining) and trains a
# small binary classifier head on top of its FROZEN latent embedding, to
...
parser = argparse.ArgumentParser(
    description='Train a binary classifier head on top of a frozen, already-trained CNC-VAE '
                "(CNC-DEC) encoder's latent embedding, evaluated via stratified k-fold CV "
...
def main():
    ...
    print('Loaded encoder: ds={} ls={} input_size={}'.format(ae.args.ds, latent_dim, ae.args.input_size))

    d = get_data(args.rna_file, args.clin_file, args.label_col, clinical_mode=args.clinical_mode)
    x_num = normalizeRNA(d['rnanp'])   # RNA-seq CPM, min-max scaled
    x_bin = d['clin']                  # binary clinical/genotype dummies

    if x_num.shape[1] + x_bin.shape[1] != ae.args.input_size:
        raise ValueError(
            'Input width {} does not match checkpoint input_size {} - '
            'check --clinical_mode matches how this encoder was trained'.format(
                x_num.shape[1] + x_bin.shape[1], ae.args.input_size)
        )

    z = ae.predict(x_num, x_bin)  # frozen - predict() already wraps eval()+no_grad internally
    y = d['y']
    ...
    fig.suptitle('CNC-DEC classifier head (latent_dim={}) - {}-fold CV, out-of-fold predictions'.format(
        latent_dim, args.n_splits))
    fig.tight_layout()
    plot_path = os.path.join(args.out, 'cncdec_classifier_curves.png')
    fig.savefig(plot_path, bbox_inches='tight')
    print('Saved', plot_path)

    result_path = os.path.join(args.out, 'cncdec_classifier_result.json')
    with open(result_path, 'w') as f:
        json.dump({
            'model': 'CNC-DEC', 'encoder': args.encoder, 'latent_dim': int(latent_dim),
