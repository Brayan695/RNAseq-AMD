...
adapted for X-DEC's dual-branch (s1 numeric / s2 binary) inputs.

Retrains a fresh xvae + XDEC under repeated K-fold resampling, maps each
...
from models.xvae import xvae as XVAEModel
...
def compute_cluster_stability(s1, s2, n_clusters, reference_labels, xvae_kwargs=None,
                              dec_kwargs=None, k=5, rep=2, seed=5192, verbose=True):
    """Retrains xvae + XDEC under repeated K-fold resampling and reports
    per-sample cluster stability against the reference (whole-cohort) fit.

    Parameters
    ----------
    s1, s2 : ndarray
        Numeric (s1) and binary (s2) feature matrices for the whole cohort.
    n_clusters : int
    reference_labels : ndarray
        Cluster labels from the whole-cohort XDEC fit, used both as the
        stability reference and to map each resample's clusters onto it.
    xvae_kwargs : dict, optional
        Keyword args forwarded to `xvae(...)` (ds1, ds2, ds12, ls, epochs, ...).
    dec_kwargs : dict, optional
        Keyword args forwarded to `XDEC.fit(...)` (maxiter, batch_size, ...).
    k, rep : int
    ...
    for i, (train_index, _) in enumerate(rkf.split(s1)):
        ...
        model = XVAEModel(s1_input_size=s1_train.shape[1], s2_input_size=s2_train.shape[1],
                          **xvae_kwargs)
        model.build_model(seed=seed + i)
        model.train(s1_train, s2_train, seed=seed + i, verbose=False)

        xdec = XDEC(model, n_clusters=n_clusters, seed=seed + i)
        y_pred_it, _, _, _ = xdec.fit(s1_train, s2_train, y=None, **dec_kwargs)
