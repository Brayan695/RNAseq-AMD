...
def load_preprocess(dataset_filename="C:/Users/braya/Downloads/cncvae_latent_embedding.csv", as_df=False, drop_sample_id=True):
    '''
    ...
    1. 'aak81Dataset_cpmdat.csv'  - 81 out of top 100 genes from feature selection using stability (Default option)
    ...
    # DEPRECATED:
#     if n_genes == 100:
#         data_fr = pd.read_csv(os.path.join('data', 'aak81Dataset_cpmdat.csv'.format(n_genes)))
...
def separate_df_two_groups(n_genes):
    '''
    Loads data and returns two dataframes for the two groups of patients based on the MGS column
    
    Inputs:
    n_genes = # top genes considered for feature stability when doing feature selection
    ...
    if n_genes == 100:
        data_fr = pd.read_csv(os.path.join('data', "C:/Users/braya/Downloads/cncvae_latent_embedding.csv".format(n_genes)))
    
    # Dropping weird first index column
    data_fr.drop('Unnamed: 0', 1, inplace=True)
    
    control_df = data_fr[data_fr['mgs_level'] == 'MGS1'].copy(deep=True)
    control_df.drop(['sample_id', 'mgs_level'], 1, inplace=True)
    
    disease_df = data_fr[data_fr['mgs_level'] == 'MGS4'].copy(deep=True)
    disease_df.drop(['sample_id', 'mgs_level'], 1, inplace=True)
