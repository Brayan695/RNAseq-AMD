...
def bayesian_optimization(X,
    ...
    for i in range(n_bootstrap):
        ...
        # Split into train_val (used for CV-based hyperparameter search below) and a held-out
        # test set. There's no separate single validation split anymore - with only ~166
        # samples, a single ~20-sample validation split is far too noisy a signal to search
        # 100+ hyperparameter combinations against (Bayesian search would just fit the noise
        # in that one split rather than anything that generalizes). 5-fold CV on X_train_val
        # gives a much more stable objective for the same search budget.
        X_train_val, X_test, y_train_val, y_test = train_test_split(X,
                                                                    y,
                                                                    stratify=y,
                                                                    test_size=0.2,
                                                                    random_state=rng.integers(500000))
        cv_splitter = StratifiedKFold(n_splits=5, shuffle=True, random_state=rng.integers(500000))

        if model_type == 'xgboost':
            # Parameter bounds
            # reg_lambda/reg_alpha/gamma upper bounds tightened from (1000, 1000, 5) - at this
            # dataset size (166 samples), reg_alpha (L1) above roughly 50-100 zeroes out every
            # leaf weight and collapses the model to predicting a constant value for every
            # sample (verified: reg_alpha=500 alone -> 1 unique prediction, AUROC exactly 0.5).
            # The old bounds let the Bayesian search land in that dead zone and report a
            # degenerate "best" model.
            ...
            def xgboost_hyperparam(eta,
                ...
                # 5-fold CV AUROC on train_val - a much less noisy search objective than
                # fitting on a train split and scoring a single ~20-sample validation split.
                auroc = cross_val_score(model, X_train_val, y_train_val,
                                        cv=cv_splitter, scoring='roc_auc').mean()
                return auroc
        ...
        # Fit on the full train_val set - no longer need to hold back a separate validation
        # split, since the hyperparameter search above used k-fold CV instead.
